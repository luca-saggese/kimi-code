...
\`tincture_calculator\` pianifica una tintura alcolica completa: due fasi di lavoro — **plan** (progetta solvente, rapporto, tempo, temperatura, preparazione) e **dose** (calcola dose batch da bench trial reale). Supporta 13 categorie con preset dipendenti dallo stato fisico dell'ingrediente.

**⚠️ REGOLE FONDAMENTALI:**
- Il bench trial è OBBLIGATORIO prima di dosare il batch. Usa \`mode:"plan"\` per progettare, poi \`mode:"dose"\` dopo aver fatto il bench trial.
- Per \`mode:"dose"\` servono OBBLIGATORIAMENTE \`beer_volume_l\`, \`test_sample_ml\`, \`test_dose_ml\`.
...
- Mai riscaldare alcol concentrato direttamente (estremamente infiammabile).
- Per la categoria "other" è OBBLIGATORIO \`food_safe_confirmed: true\`.
- Il tool valida automaticamente la compatibilità categoria–stato (es. hop solo pellet/whole, wood solo chips/cubes).
- Segnala automaticamente parametri fuori dai preset consigliati (ABV, tempo, temperatura).

**Categorie e stati compatibili:**

| Categoria | Stati | ABV solvente | Rapporto (varia per stato) | Tempo (varia per stato) |
|---|---|---|---|---|
| hop | pellet, whole | 45–55% (standard) / 60–70% (cold_short) | 1:10 pellet, 1:12 coni | 12–48h / 4–12h |
| wood | chips, cubes | 45–65% | 1:7 chips, 1:5 cubes | 3–14gg / 14–42gg |
| seed_spice | whole, crushed, ground | 45–60% | 1:8–1:15 | 6h–10gg |
| bark_root | whole, crushed | 50–70% | 1:8–1:10 | 3–30gg |
| fresh_herb | fresh | 55–70% | 1:3 | 4–48h |
| dried_herb | dried, crushed | 35–55% | 1:15–1:20 | 6h–7gg |
| citrus_peel | fresh, dried | 60–75% | 1:5 fresh, 1:12 dried | 12h–7gg |
| chili | fresh, dried, crushed | 60–75% | 1:10 fresh, 1:20 dried | 6h–7gg |
| coffee | whole, ground | 20–40% | 1:4 whole, 1:7 ground | 1–3gg / 12–48h |
| cacao | whole, crushed | 45–60% | 1:4 | 3–21gg |
| vanilla | whole | 40–60% | ~3g baccello / 75mL (1:25) | 14–60gg |
| fruit | fresh, dried | 60–75% | 1:0.67 fresh, 1:4 dried | 3–14gg |
| other | tutti | 40–60% | 1:10 | 3–14gg |

**Parametri principali (mode: plan):**
- \`ingredient\`: nome dell'ingrediente
- \`category\`: hop, wood, seed_spice, bark_root, fresh_herb, dried_herb, citrus_peel, chili, coffee, cacao, vanilla, fruit, other
- \`ingredient_weight_g\`: peso in grammi
- \`ingredient_state\`: compatibile con la categoria (vedi tabella)
- \`target_abv_percent\`: gradazione target (opzionale, default da preset; non può superare source_abv_percent)
- \`source_abv_percent\`: gradazione alcol di partenza (default 95)
- \`hop_variant\`: "standard" o "cold_short" (solo per hop)
- \`extraction_time_days\`, \`extraction_temp_c\`: opzionali (fuori preset → warning)
- \`ingredient_water_percent\`: contenuto acqua ingrediente (stima diluizione solvente)
- \`ingredient_sugar_percent\`: per avviso fermentabili
- \`custom_ratio\`: rapporto personalizzato g/mL (NON usare insieme a solvent_volume_ml)
- \`food_safe_confirmed\`: OBBLIGATORIO per categoria "other"

**Parametri per mode: dose (dopo bench trial):**
- \`mode: "dose"\`
- \`beer_volume_l\`: volume EFFETTIVO birra nel fermentatore/keg
- \`test_sample_ml\`: volume campione usato nel bench trial
- \`test_dose_ml\`: dose scelta nel campione (mL)
- \`recovered_tincture_volume_ml\`: volume tintura realmente recuperato dopo filtrazione

**Esempi:**
- Fase 1 — plan: \`tincture_calculator({mode:"plan", ingredient:"Luppolo Citra", category:"hop", ingredient_weight_g:10, ingredient_state:"pellet", target_abv_percent:50})\`
- Fase 1 — plan cold_short: \`tincture_calculator({mode:"plan", ingredient:"Citra", category:"hop", ingredient_weight_g:10, ingredient_state:"pellet", hop_variant:"cold_short", extraction_temp_c:4})\`
- Fase 1 — plan legno: \`tincture_calculator({mode:"plan", ingredient:"Quercia francese", category:"wood", ingredient_weight_g:20, ingredient_state:"chips", target_abv_percent:55, extraction_time_days:7})\`
- Fase 1 — plan spezie: \`tincture_calculator({mode:"plan", ingredient:"Coriandolo", category:"seed_spice", ingredient_weight_g:15, ingredient_state:"crushed", extraction_time_days:3})\`
- Fase 1 — plan scorze: \`tincture_calculator({mode:"plan", ingredient:"Scorza d'arancia", category:"citrus_peel", ingredient_weight_g:20, ingredient_state:"fresh", target_abv_percent:70})\`
- Fase 1 — plan vaniglia: \`tincture_calculator({mode:"plan", ingredient:"Bacca vaniglia", category:"vanilla", ingredient_weight_g:3, ingredient_state:"whole", extraction_time_days:30})\`
- Fase 2 — dose: \`tincture_calculator({mode:"dose", ingredient:"Citra", category:"hop", ingredient_weight_g:10, ingredient_state:"pellet", beer_volume_l:20, test_sample_ml:100, test_dose_ml:0.15, recovered_tincture_volume_ml:55})\`

**Usalo SEMPRE per:** pianificare qualsiasi tintura alcolica per birra; determinare ABV solvente, rapporto e tempo per ogni categoria e stato; verificare la sicurezza dell'ingrediente; calcolare la dose batch DOPO aver completato il bench trial; valutare il contributo alcolico della tintura sulla birra finita.
